...
extendLocale('zh-CN', {
  ...
  'e6f158f9a9d3f173ac7ef6821b409ba3':
    "比如可以传入 'day'、'month'、'year' 或者 `week` 等等",
  ...
  '75d4f99310ce6d885dfca27342babf3c':
    '如果满足条件condition，则返回consequent，否则返回alternate，支持多层嵌套IF函数。 等价于直接用JS表达式如：condition ? consequent : alternate。',
  '33136429f20612f8bbe72998c71ee8fe':
    '条件全部符合，返回 true，否则返回 false。 示例：AND(语文成绩>80, 数学成绩>80)， 语文成绩和数学成绩都大于 80，则返回 true，否则返回 false， 等价于直接用JS表达式如：语文成绩>80 && 数学成绩>80。',
  '1aeeb99a79d7157e0f0ddd28298e9323':
    '条件任意一个满足条件，返回 true，否则返回 false。 示例：OR(语文成绩>80, 数学成绩>80)， 语文成绩和数学成绩任意一个大于 80，则返回 true，否则返回 false， 等价于直接用JS表达式如：语文成绩>80 || 数学成绩>80。',
  '171db2606fa6338498d61e060fb35fa3':
    '异或处理，多个表达式组中存在奇数个真时认为真。 示例：XOR(语文成绩 > 80, 数学成绩 > 80, 英语成绩 > 80) 三门成绩中有一门或者三门大于 80，则返回 true，否则返回 false。',
  '4682a020956e97b8c0b7dc44cbc8e6b9':
    '判断函数集合，相当于多个 else if 合并成一个。 示例：IFS(语文成绩 > 80, "优秀", 语文成绩 > 60, "良", "继续努力")， 如果语文成绩大于 80，则返回优秀，否则判断大于 60 分，则返回良，否则返回继续努力。',
  ...
  '546906596299e8cb04c44a8cd2e23ae5':
    '返回大于等于 0 且小于 1 的均匀分布随机实数。每一次触发计算都会变化。 示例：`RAND()*100`， 返回 0-100 之间的随机数。',
  ...
  '9a8dedae5832076dbaed75bb34e7fc24':
    '返回计算机字符集的数字代码所对应的字符。 示例：`CHAR(97)` 等价于 "a"。',
  ...
  'a8d68e49bcd165601e928772821ef9c6':
    '返回文本字符串中从指定位置开始的特定数目的字符。 示例：`MID("amis.baidu.com", 6, 3)`， 返回 `aid`。',
  'fad1d635755dd8fb26d90b1ca0da003c':
    '返回路径中的文件名。 示例：`/home/amis/a.json`， 返回：`a.json`。',
  '045b831dd1fd3f25be4f1977bec9ef25': '生成UUID字符串',
  'fdd264ef6ebaadcdad6670dd782ebaaa':
    '创建日期对象，可以通过特定格式的字符串，或者数值。 需要注意的是，其中月份的数值是从0开始的， 即如果是12月份，你应该传入数值11。',
  ...
  'fc4527e4de2f4b00368f124759ee7f47':
    "获取日期的星期几。 示例 WEEKDAY('2023-02-27') 得到 0。 WEEKDAY('2023-02-27', 2) 得到 1。",
  '653e1995b9360d5ea17adc62561fa1f2':
    "获取年份的星期，即第几周。 示例 WEEK('2023-03-05') 得到 9。",
  '3d4ceadcf373d7f9ff18de20ccedd48d':
    "对日期、日期字符串、时间戳进行格式化。 示例 DATETOSTR('12/25/2022', 'YYYY-MM-DD') 得到 '2022.12.25'， DATETOSTR(1676563200, 'YYYY.MM.DD') 得到 '2023.02.17'， DATETOSTR(1676563200000, 'YYYY.MM.DD hh:mm:ss') 得到 '2023.02.17 12:00:00'， DATETOSTR(DATE('2021-12-21'), 'YYYY.MM.DD hh:mm:ss') 得到 '2021.12.21 08:00:00'。",
  ...
  '56b264e217aacf9fae37934497fae6bd':
    "修改日期，对日期进行加减天、月份、年等操作。 示例： DATEMODIFY(A, -2, 'month')， 对日期 A 进行往前减2月的操作。",
  '7e2e144d81fb33ce97d3cfeedb3651c1':
    "将字符日期转成日期对象，可以指定日期格式。 示例：STRTODATE('2021/12/6', 'YYYY/MM/DD')",
  ...
  'f7728fb4212ce3d9dda0957df14d6fc5':
    "判断日期是否在指定范围内，是则返回 true，否则返回 false。 示例：BETWEENRANGE('2021/12/6', ['2021/12/5','2021/12/7'])。",
  ...
  '61533e70a6220dea82052941e690c4b8':
    '数组做数据转换，需要搭配箭头函数一起使用，注意箭头函数只支持单表达式用法。 将数组中的每个元素转换成箭头函数返回的值。 示例： ARRAYMAP([1, 2, 3], item => item + 1) 得到 [2, 3, 4]。',
  '3288dcc0e39872c6fd07911fd5946c4f':
    '数据做数据过滤，需要搭配箭头函数一起使用，注意箭头函数只支持单表达式用法。 将第二个箭头函数返回为 false 的成员过滤掉。 示例： ARRAYFILTER([1, 2, 3], item => item > 1) 得到 [2, 3]。',
  'e9dba27896aa5a79fd794a61c64c3124':
    '数据做数据查找，需要搭配箭头函数一起使用，注意箭头函数只支持单表达式用法。 找出第二个箭头函数返回为 true 的成员的索引。 示例： ARRAYFINDINDEX([0, 2, false], item => item === 2) 得到 1。',
  '669d6b5340fc55359ed36bad85ca3c37':
    '数据做数据查找，需要搭配箭头函数一起使用，注意箭头函数只支持单表达式用法。 找出第二个箭头函数返回为 true 的成员。 示例： ARRAYFIND([0, 2, false], item => item === 2) 得到 2。',
  '3785b0ca5ae801a018c941eb5ab7b541':
    '数据做数据遍历判断，需要搭配箭头函数一起使用，注意箭头函数只支持单表达式用法。 判断第二个箭头函数是否存在返回为 true 的成员，是则返回 true，否则返回 false。 示例： ARRAYSOME([0, 2, false], item => item === 2) 得到 true。',
  '2e47575507eb5ebadb9bd5ffc01ac12c':
    '数据做数据遍历判断，需要搭配箭头函数一起使用，注意箭头函数只支持单表达式用法。 判断第二个箭头函数返回是否都为 true，是则返回 true，否则返回 false。 示例： ARRAYEVERY([0, 2, false], item => item === 2) 得到 false',
  '27e200e541b31e5aca38eb7d8f12667f':
    '判断数据中是否存在指定元素。 示例： ARRAYINCLUDES([0, 2, false], 2) 得到 true。',
  '7ada9f67416ea16540fcb61725e2397f':
    '数组过滤掉 false、null、0 和 ""。 示例： COMPACT([0, 1, false, 2, \'\', 3]) 得到 [1, 2, 3]。',
  '0b82c69331128197f5653deabe707aee':
    "数组转成字符串。 示例： JOIN(['a', 'b', 'c'], '=') 得到 'a=b=c'。",
  'a2ef05f1e4a8af0d5c380307d2cc3cce':
    "数组合并。 示例： CONCAT(['a', 'b', 'c'], ['1'], ['3']) 得到 ['a', 'b', 'c', '1', '3']。",
  '3c227600a4af0574e8782e96455862a9':
    "数组去重，第二个参数「field」，可指定根据该字段去重。 示例： UNIQ([{a: '1'}, {b: '2'}, {a: '1'}]) 得到 [{a: '1'}, {b: '2'}]。",
  'b90e85a6f375f73dcc262c0803fbf7af':
    '将JS对象转换成JSON字符串。 示例： ENCODEJSON({name: \'amis\'}) 得到 \'{\\"name\\":\\"amis\\"}\'。',
  '3effcacff99f8e3a6c560990bc0c5e7b':
    '解析JSON编码数据，返回JS对象。 示例： DECODEJSON(\'{\\"name\\": "amis"}\') 得到 {name: \'amis\'}。',
